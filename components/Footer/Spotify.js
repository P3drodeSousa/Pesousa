  ...
  return (
    <div className="flex flex-1 w-full items-center justify-center my-2 md:my-0">
      <div className="flex flex-col justify-center items-center sm:flex-row w-full truncate">
        ...
        {data?.songUrl ? (
          <a
            className="text-gray-800 dark:text-white font-medium  max-w-max truncate"
            href={data.songUrl}
            target="_blank"
          ...
          <p className="text-black dark:text-white font-medium">Not Playing</p>
        )}
        <span className="mx-2 text-gray-500 dark:text-gray-300 hidden sm:block">
          {" – "}
        </span>
        <p className="text-black dark:text-white max-w-max truncate">
          {data?.artist ?? "Spotify"}
        </p>
      </div>
    </div>
